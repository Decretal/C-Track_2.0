...
#define DO			(PINB &	(1 << PB6))	/* Test for MMC DO ('H':true, 'L':false) */

#define DI_INIT()	DDRB  |= (1 << PINB5)	/* Initialize port for MMC DI as output */

#define CK_INIT()	DDRB  |= (1 << PINB7)	/* Initialize port for MMC SCLK as output */
#define CK_H()		PORTB |= (1 << PB7)	/* Set MMC SCLK "high" */
#define	CK_L()		PORTB &= ~(1 << PB7)	/* Set MMC SCLK "low" */

#define CS_INIT()	DDRB  |= (1 << PINB4)	/* Initialize port for MMC CS as output */
#define	CS_H()		PORTB |= (1 << PB4)	/* Set MMC CS "high" */
#define CS_L()		PORTB &= ~(1 << PB4)	/* Set MMC CS "low" */
